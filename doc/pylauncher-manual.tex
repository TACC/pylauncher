...
cluster of choice. Inside your batch script, you execute a launcher
script with python:
\begin{verbatim}
...
\subsection{Classic launcher mode}
...
\subsection{Launching MPI jobs}

What if you have a large number of MPI jobs that need a relatively
small core count? The python launcher can accomodate this with 
\begin{verbatim}
...
... et cetera
\end{verbatim}
Unfortunately in this mode the batch output file will contain a lot of
noise, since each commandline is treated as a separate parallel job,
submitted with \n{ibrun}.

\subsection{A dynamic launcher}
...
You can trace the workings of the launcher, to see when jobs start and
finish, and where they are scheduled, by adding a \n{debug} value to
the launcher call:
\begin{verbatim}
